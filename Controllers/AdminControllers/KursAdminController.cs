...
namespace EgitimSitesi.Controllers.AdminControllers
{
    ...
    public class KursAdminController : Controller
    {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (imageFile != null && imageFile.Length > 0)
                    {
                        ...
                        if (!string.IsNullOrEmpty(existingKurs.Image))
                        {
                            var oldImagePath = Path.Combine(_environment.WebRootPath, existingKurs.Image.TrimStart('/'));
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            if (!string.IsNullOrEmpty(kurs.Image))
            {
                var imagePath = Path.Combine(_environment.WebRootPath, kurs.Image.TrimStart('/'));
